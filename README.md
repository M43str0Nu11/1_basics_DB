...
Скорее всего вы сильно испугались увидев такую большую таблицу, но для начала работы мы будем использовать реляционные БД, в частности SQL Server.
Возможно вы сечйчас спросите почему именно SQL Server, а не другие?

*Ответ просто как 3 рубля, я на работе и на учёбе постоянно работаю с SQL Server и уверен с ним работаю* :)
...
CREATE TABLE Студенческий_билет (
    ...
    Студент_ID INT UNIQUE,  -- UNIQUE гарантирует 1:1
...
CREATE TABLE Студент (
    ...
    FOREIGN KEY (Группа_ID) REFERENCES Группа(ID)
);
```
![СТуденты](pict8.png)
...
CREATE TABLE Студент_Дисциплина (
    ...
    Оценка INT,
    FOREIGN KEY (Студент_ID) REFERENCES Студент(ID),
    FOREIGN KEY (Дисциплина_ID) REFERENCES Дисциплина(ID)
);
```
![СТуденты](pict9.png)
